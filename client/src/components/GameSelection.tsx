...
interface GameSelectionProps {
  room: Room;
  onGameSelected: (room: unknown, gameType: string) => void;
  onClose: () => void;
}

const GameSelection: React.FC<GameSelectionProps> = ({ room, onGameSelected, onClose }) => {
  const [availableGames, setAvailableGames] = useState<Record<string, Game>>({});
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string>('');
  const [fetchingGames, setFetchingGames] = useState<boolean>(true);

  useEffect(() => {
    fetchAvailableGames();
  }, []);

  const fetchAvailableGames = async (): Promise<void> => {
    try {
  ...
  const handleGameSelect = async (gameType: string): Promise<void> => {
    setLoading(true);
    setError('');
