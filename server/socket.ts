    ...
    async (
      ...
      socket.on('createRoom', async data => {
        try {
          ...
          const participant: RoomMember = await db.addParticipant(
            room.id,
            user.id,
            socket.id,
            'host'
          ...
          try {
            ...
            if (clientIsHostHint && !roomHasHost && user && user.id) {
              ...
              await db.adminClient.from('rooms').update({ host_id: user.id }).eq('id', room.id);
      ...
      socket.on('joinSocketRoom', data => {
        try {
          logger.socket('Joining socket room for listening', {
        ...
        try {
          ...
          try {
            ...
            const connectedPlayers =
              room.participants?.filter((p: RoomMember) => p.is_connected === true).length || 0;
            ...
            const existingParticipant = room.participants?.find(
              (p: RoomMember) => p.user_id === user.id || p.user?.username === playerName
            );
          ...
          if (connection && connection.roomCode) {
            ...
            if (room) {
              const participant = room.participants?.find(
                (p: RoomMember) => p.socket_id === socket.id
              );
              if (participant) {
                // Mark as disconnected
                await db.updateParticipant(participant.id, {
