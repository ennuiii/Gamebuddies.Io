  ...
  socket.on(SOCKET_EVENTS.CHAT.MESSAGE, async (data) => {
    ...
    if (connection?.userId && connection?.roomId) {
      try {
        ...
        if (participant) {
          playerName = (participant as any).custom_lobby_name
            || (participant as any).user?.display_name
            || 'Player';
          console.log('💬 [CHAT] Resolved player name from DB:', playerName);
        }
      } catch (err) {
        console.error('❌ [CHAT] Error looking up player name:', (err as Error).message);
      }
    }
