...
module.exports = (io, db, connectionManager, lobbyManager, statusSyncManager) => {
      ...
      try {
        ...
        if (shouldCreateSession) {
          try {
            ...
            await db.adminClient.from('game_sessions').insert({
        ...
        try {
          await db.logEvent(room.id, null, 'external_return_to_lobby', {
        ...
        const returnUrl =
          room.streamer_mode && sessionToken
  ...
  router.get(
    '/api/v2/rooms/:roomCode/return-status',
    apiKeyMiddleware,
    rateLimits.polling,
    async (req, res) => {
      try {
  ...
  router.get(
    '/api/v2/rooms/:roomCode/validate-with-session',
    apiKeyMiddleware,
    rateLimits.apiCalls,
    async (req, res) => {
      try {
  ...
  router.post(
    '/api/v2/external-heartbeat',
    apiKeyMiddleware,
    rateLimits.heartbeats,
    async (req, res) => {
      try {
...
async function validateRoom(roomCode, playerId, playerName, req, db) {
  ...
  if (playerName || playerId) {
    participant = room.participants?.find(
      p => (playerName && p.user?.username === playerName) || (playerId && p.user_id === playerId)
    );
  }
