...
interface SubscriptionWithMetadata extends Stripe.Subscription {
  metadata: {
    supabase_user_id: string;
  };
 ...
 */
async function fetchStripePrices(): Promise<StripePrices> {
  ...
  try {
    ...
    let lifetimePrice: string | null = null;
    ...
    for (const price of prices.data) {
      ...
      if (price.type === 'one_time' &&
        ...
        console.log(`  ⭐ Found LIFETIME price: ${price.id} - ${product.name} (${(price.unit_amount || 0) / 100} ${price.currency})`);
      }
      ...
      if (price.type === 'recurring' &&
        ...
        console.log(`  💎 Found MONTHLY price: ${price.id} - ${product.name} (${(price.unit_amount || 0) / 100} ${price.currency}/month)`);
      }
    }
    ...
    console.error('❌ [STRIPE PRICES] Error fetching prices:', (error as Error).message);
...
fetchStripePrices()
  ...
  .catch(error => {
    console.error('❌ [STRIPE CONFIG] Failed to fetch prices on startup:', (error as Error).message);
 ...
 */
router.post('/create-checkout-session', requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (req.user!.id !== userId) {
      ...
      res.status(403).json({ error: 'Forbidden - Can only create checkout for yourself' });
      return;
    }
    ...
    if (referralCode) {
      ...
      if (affiliate) {
        affiliateId = (affiliate as { id: string }).id;
    ...
    if (!['lifetime', 'monthly'].includes(priceType)) {
      res.status(400).json({ error: 'priceType must be "lifetime" or "monthly"' });
      return;
    }
    ...
    if (typedUser.premium_tier === 'lifetime') {
      res.status(400).json({ error: 'User already has lifetime premium' });
      return;
    }

    if (typedUser.premium_tier === 'monthly' && priceType === 'monthly') {
      res.status(400).json({ error: 'User already has an active monthly subscription' });
      return;
    }
...
router.get('/prices', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const lifetimeProduct = lifetimeDetails.product as Stripe.Product;
    const monthlyProduct = monthlyDetails.product as Stripe.Product;
    ...
    console.error('❌ [STRIPE] Error fetching prices:', (error as Error).message);
    res.status(500).json({
  ...
  try {
    switch (event.type) {
      ...
      case 'customer.subscription.updated':
        await handleSubscriptionUpdate(event.data.object as SubscriptionWithMetadata);
        break;

      case 'customer.subscription.deleted':
        await handleSubscriptionDeleted(event.data.object as SubscriptionWithMetadata);
        break;
 ...
 */
router.post('/customer-portal', requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (req.user!.id !== userId) {
      ...
      res.status(403).json({ error: 'Forbidden - Can only access your own customer portal' });
      return;
    }
    ...
    if (userError || !user || !(user as { stripe_customer_id: string | null }).stripe_customer_id) {
      res.status(404).json({ error: 'User or customer not found' });
      return;
    }

    // Create portal session
    const session = await stripe.billingPortal.sessions.create({
      customer: (user as { stripe_customer_id: string }).stripe_customer_id,
 ...
 */
router.get('/validate-referral/:code', async (req: Request, res: Response): Promise<void> => {
 ...
 */
router.post('/cancel-subscription', requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { userId } = req.body;
    ...
    if (req.user!.id !== userId) {
      ...
      res.status(403).json({ error: 'Forbidden - Can only cancel your own subscription' });
      return;
    }
    ...
    if (userError || !user || !(user as { stripe_subscription_id: string | null }).stripe_subscription_id) {
    ...
    const subscription = await stripe.subscriptions.update(
      (user as { stripe_subscription_id: string }).stripe_subscription_id,
    ...
    res.json({
      ...
      endsAt: new Date((subscription as unknown as SubscriptionWithMetadata).current_period_end * 1000)
    });
  } catch (error) {
...
async function handleCheckoutCompleted(session: CheckoutSession): Promise<void> {
  ...
  if (affiliateId) {
    try {
      ...
      if (affiliate) {
        const typedAffiliate = affiliate as { commission_rate: number; total_earnings: number };
        const commission = amountPaid * (typedAffiliate.commission_rate || 0.20);
        ...
        await supabaseAdmin.from('affiliates').update({
          total_earnings: (Number(typedAffiliate.total_earnings) || 0) + commission
        }).eq('id', affiliateId);
        ...
        if (referralError) {
          console.log('ℹ️ User already referred:', referralError.message); // Safe ignore
        }
  ...
  if (session.mode === 'subscription' && session.subscription) {
    ...
    const subscription = await stripe.subscriptions.retrieve(session.subscription as string) as unknown as SubscriptionWithMetadata;
 ...
 */
async function handleSubscriptionUpdate(subscription: SubscriptionWithMetadata): Promise<void> {
 ...
 */
async function handleSubscriptionDeleted(subscription: SubscriptionWithMetadata): Promise<void> {
...
async function handlePaymentSucceeded(invoice: Stripe.Invoice): Promise<void> {
  ...
  if (subscriptionId) {
    ...
    await handleSubscriptionUpdate(subscription as unknown as SubscriptionWithMetadata);
  }
}
...
async function handlePaymentFailed(invoice: Stripe.Invoice): Promise<void> {
  ...
  if (subscriptionId) {
    const subscription = await stripe.subscriptions.retrieve(subscriptionId as string);
    const userId = (subscription as unknown as SubscriptionWithMetadata).metadata.supabase_user_id;
