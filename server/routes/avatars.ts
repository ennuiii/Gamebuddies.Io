    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      try {
        ...
        if (user && !error) {
          ...
          if (publicUser && (publicUser as { role: string }).role === 'admin') {
            isAdmin = true;
          }
        }
      } catch (authError) {
        console.warn('Error checking admin status:', (authError as Error).message);
        // Continue as non-admin
      }
    }

    // Helper to scan directory with level support
    const scanDir = (type: string, isPremium: boolean): void => {
