...
const apiKeyMiddleware =
  typeof validateApiKey === 'function' ? validateApiKey : (req, res, next) => next();
...
module.exports = (io, db, connectionManager) => {
  ...
  router.get(
    '/rooms/:roomCode/validate',
    apiKeyMiddleware,
    getRateLimiter('apiCalls'),
    async (req, res) => {
      try {
        ...
        console.log(
          `ðŸ” [API V2] Validating room ${roomCode} for service ${req.apiKey.service_name}`
        );
        ...
        if (playerName || playerId) {
          ...
          if (sessionToken && participant?.session?.length > 0) {
            const activeSession = participant.session.find(
              s =>
  ...
  router.post(
    '/rooms/:roomCode/bulk-status',
    apiKeyMiddleware,
    getRateLimiter('bulkUpdates'),
    async (req, res) => {
      try {
        const { roomCode } = req.params;
        ...
        console.log(
          `ðŸ“¦ [API V2] Bulk updating ${players?.length || 0} players in room ${roomCode}`
        );
        ...
        console.log(
          `[API V2] Bulk update completed: ${result.summary.successful}/${result.summary.total} successful`
        );
  ...
  router.post('/sessions/recover', async (req, res) => {
    try {
      ...
      console.log(
        `ðŸ”„ [API V2] Attempting session recovery for token: ${sessionToken.substring(0, 8)}...`
      );

      const result = await lobbyManager.recoverSession(
        sessionToken,
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        let hasHost =
          Array.isArray(room.participants) && room.participants.some(p => p.role === 'host');
        if (!hasHost) {
          ...
          if (hostUserId) {
            ...
            await db.adminClient.from('rooms').update({ host_id: hostUserId }).eq('id', room.id);
  ...
  router.post(
    '/rooms/:roomCode/players/:playerId/heartbeat',
    apiKeyMiddleware,
    getRateLimiter('heartbeats'),
    async (req, res) => {
      try {
        const { roomCode, playerId } = req.params;
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        // Log event
        await db.logEvent(
          (await db.adminClient.from('rooms').select('id').eq('room_code', roomCode).single()).data
            ?.id,
          null,
          'game_ended_api_v2',
          {
