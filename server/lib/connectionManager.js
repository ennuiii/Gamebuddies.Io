...
class ConnectionManager {
  ...
  getRoomConnections(roomId) {
    return Array.from(this.activeConnections.values()).filter(conn => conn.roomId === roomId);
  }

  // Get all connections for a user
  getUserConnections(userId) {
    return Array.from(this.activeConnections.values()).filter(conn => conn.userId === userId);
  }
    ...
    return {
      ...
      averageConnectionAge:
        connections.length > 0
          ? connections.reduce((sum, c) => sum + (now - c.connectedAt.getTime()), 0) /
            connections.length /
            1000
          : 0,
