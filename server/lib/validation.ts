...
import Joi, { Schema, ValidationResult as JoiValidationResult } from 'joi';
...
let validGameTypesCache: string[] | null = null;
let cacheTimestamp = 0;
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes

async function getValidGameTypes(): Promise<string[]> {
  const now = Date.now();
  ...
  try {
    ...
    // Update cache
    validGameTypesCache = [...(games?.map((g: { id: string }) => g.id) || []), 'lobby']; // 'lobby' is always valid
    cacheTimestamp = now;
...
const gameTypeSchema = Joi.string()
  .external(async (value: string | undefined) => {
...
function createValidator(schemaName: string): (data: unknown) => Promise<ValidationResult> {
  return async (data: unknown): Promise<ValidationResult> => {
    ...
    try {
      ...
      })) : [{ field: 'unknown', message: (error as Error).message }];
...
const rateLimits: Record<string, RateLimitConfig | RateLimitRequestHandler> = {
...
const apiRateLimiterConfig: Record<string, { windowMs: number; max: number }> = {
...
const createRateLimiter = (config: Partial<RateLimitOptions> = {}): RateLimitRequestHandler => rateLimit({
...
async function validateApiKey(
  ...
  try {
    ...
    req.apiKey = keyRecord as ApiKeyRecord;
...
function clearGameTypesCache(): void {
