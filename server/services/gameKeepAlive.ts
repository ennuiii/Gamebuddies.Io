...
interface PingResultWithGame extends PingResult {
  game: string;
}
...
class GameKeepAliveService {
   ...
   */
  async pingGame(game: Game): Promise<PingResult> {
   ...
   */
  async pingAllGames(): Promise<void> {
    ...
    const results: PingResultWithGame[] = [];
...
export default new GameKeepAliveService();
