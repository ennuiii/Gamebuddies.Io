...
exports.clearHostTransferGracePeriod = clearHostTransferGracePeriod;
...
const hostTransferGracePeriods = new Map();
/**
 * Start a grace period before transferring host after disconnect
 */
function startHostTransferGracePeriod(ctx, roomId, roomCode, disconnectedHostId) {
    ...
    const timeout = setTimeout(async () => {
        try {
            ...
            const originalHostParticipant = room.participants?.find((p) => p.user_id === disconnectedHostId);
 ...
 */
function initializeSocketIO(httpServer, ctx) {
    ...
    io.on('connection', async (socket) => {
        ...
        (0, chatHandlers_1.registerChatHandlers)(socket, fullCtx, gameState);
        (0, friendHandlers_1.registerFriendHandlers)(socket, fullCtx);
        (0, connectionHandlers_1.registerConnectionHandlers)(socket, fullCtx);
        (0, roomHandlers_1.registerRoomHandlers)(socket, fullCtx);
        (0, gameHandlers_1.registerGameHandlers)(socket, fullCtx);
        (0, playerHandlers_1.registerPlayerHandlers)(socket, fullCtx);
        ...
        socket.on('disconnect', async () => {
            try {
                ...
                if (connection?.userId) {
                    ...
                    await (0, friendHandlers_1.notifyFriendsOffline)(fullCtx, connection.userId);
                    ...
                    await fullCtx.db.updateParticipantConnection(connection.userId, socket.id, connectionStatus);
                    // Auto-update room status if host
                    if (isDisconnectingHost && connectionStatus && room) {
                        await (0, roomStatusService_1.autoUpdateRoomStatusByHost)(io, fullCtx.db, room.id, connection.userId, connectionStatus);
                    }
                    // Handle host disconnect with grace period
                    if (isDisconnectingHost && room) {
                        const otherConnectedPlayers = room.participants?.filter((p) => p.user_id !== connection.userId && p.is_connected === true) || [];
 ...
 */
function clearHostTransferGracePeriod(roomId) {
