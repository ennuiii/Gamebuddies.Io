...
const roomStatusService_1 = require("../services/roomStatusService");
/**
 * Map participants to player format for client
 */
function mapParticipantsToPlayers(participants) {
 ...
 */
function registerRoomHandlers(socket, ctx) {
    const { io, db, connectionManager, roomLifecycleManager } = ctx;
    // Handle room creation
    socket.on('createRoom', async (data) => {
        try {
            // Validate input
            const validation = await validation_1.validators.createRoom(data);
            ...
            // Check rate limiting
            const createRoomLimit = validation_1.rateLimits.createRoom;
            ...
            // Sanitize input
            const playerName = validation_1.sanitize.playerName(data.playerName);
            const customLobbyName = data.customLobbyName ? validation_1.sanitize.playerName(data.customLobbyName) : null;
            ...
            if (supabaseUserId) {
                ...
                user = await db.getOrCreateUser(`${socket.id}_${playerName}`, playerName, playerName, { is_guest: true });
            }
    ...
    socket.on('getPublicRooms', async (data) => {
        ...
        `)
            ...
            const activeRooms = (rooms || []).filter((room) => {
                ...
                const recentlyActiveMembers = room.members?.filter((m) => m.is_connected && new Date(m.last_ping) > fiveMinutesAgo) || [];
    ...
    socket.on('joinSocketRoom', (data) => {
        try {
            const roomCode = validation_1.sanitize.roomCode(data?.roomCode);
    ...
    socket.on('joinRoom', async (data) => {
        try {
            // Validate input
            const validation = await validation_1.validators.joinRoom(data);
            if (!validation.isValid) {
                socket.emit('error', {
                    message: validation.message,
                    code: 'INVALID_INPUT'
            ...
            // Check rate limiting
            const joinRoomLimit = validation_1.rateLimits.joinRoom;
            ...
            // Sanitize input
            const playerName = validation_1.sanitize.playerName(data.playerName);
            let customLobbyName = data.customLobbyName ? validation_1.sanitize.playerName(data.customLobbyName) : null;
            const roomCode = validation_1.sanitize.roomCode(data.roomCode);
            const supabaseUserId = data.supabaseUserId || null;
            ...
            try {
                ...
                if (room.status === 'abandoned') {
                    ...
                    if (!isPreviousParticipant && data.playerName) {
                        isPreviousParticipant = room.participants?.some(p => p.user?.username === data.playerName ||
                            p.user?.display_name === data.playerName ||
                            p.custom_lobby_name === data.playerName) || false;
                    }
                }
                ...
                if (!existingParticipant && data.playerName) {
                    existingParticipant = room.participants?.find(p => p.user?.username === data.playerName ||
                        p.user?.display_name === data.playerName ||
                ...
                if (existingParticipant) {
                    ...
                    if (existingParticipant.role === 'host') {
                        await (0, roomStatusService_1.autoUpdateRoomStatusByHost)(io, db, room.id, existingParticipant.user_id, 'lobby');
                    }
                    ...
                    if (supabaseUserId) {
                        ...
                        user = await db.getOrCreateUser(`${socket.id}_${data.playerName}`, data.playerName, data.playerName);
                    }
                    // Check for duplicate connected participants
                    const duplicateConnectedParticipant = room.participants?.find(p => p.user?.username === data.playerName &&
                        p.is_connected === true &&
                ...
                if (updatedRoom.status === 'in_game') {
                    await (0, roomStatusService_1.autoUpdateRoomStatusBasedOnPlayerStates)(io, db, updatedRoom, players, 'player_rejoined');
                }
            }
            catch (error) {
