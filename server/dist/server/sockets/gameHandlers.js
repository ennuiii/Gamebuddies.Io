 ...
 */
function registerGameHandlers(socket, ctx) {
    ...
    socket.on('selectGame', async (data) => {
        try {
            ...
            // Validate gameType
            const validation = await validation_1.validators.selectGame({
            ...
            const cleanSettings = validation_1.sanitize.gameSettings(data?.settings || {});
    ...
    socket.on('startGame', async (data) => {
        ...
        try {
            ...
            // Verify user is host
            const userParticipant = room.participants?.find(p => p.user_id === connection.userId && p.role === 'host');
            if (!userParticipant) {
