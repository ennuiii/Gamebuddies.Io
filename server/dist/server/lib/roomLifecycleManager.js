    ...
    startAbandonmentGracePeriod(roomId, roomCode) {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                const { data: room, error } = await supabase_1.db.adminClient
                    .from('rooms')
    ...
    startHostTransferGracePeriod(roomId, roomCode, originalHostUserId) {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                const otherConnectedPlayers = room?.room_members?.filter((m) => m.user_id !== originalHostUserId && m.is_connected === true) || [];
                if (otherConnectedPlayers.length > 0) {
                    console.log(`👑 [HOST] Grace period expired, original host not connected - transferring host`);
                    const newHost = await supabase_1.db.autoTransferHost(roomId, originalHostUserId);
                    if (newHost) {
...
exports.default = RoomLifecycleManager;
//# sourceMappingURL=roomLifecycleManager.js.map