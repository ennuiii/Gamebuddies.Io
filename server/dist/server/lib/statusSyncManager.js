...
class StatusSyncManager {
    ...
    async reconcileStatusConflicts(playerId, roomCode, serverStatus, clientStatus) {
        ...
        `)
            ...
            const dbState = currentState;
            // Determine resolution strategy
            const resolution = this.resolveConflict(dbState, serverStatus, clientStatus);
            // Apply resolution
            if (resolution.updateRequired) {
                await this.lobbyManager.updatePlayerStatus(playerId, roomCode, resolution.resolvedStatus.status, resolution.resolvedStatus.location, {
    ...
    async processStatusUpdate(updateKey) {
        ...
        if (!update)
            return;
        await this.lobbyManager.updatePlayerStatus(update.playerId, update.roomCode, update.status, update.location, update.metadata);
    }
    ...
    resolveConflict(dbState, serverStatus, clientStatus) {
        ...
        if (clientStatus.isConnected !== undefined &&
            serverStatus.inGame !== undefined) {
            return {
    ...
    async handleGameEnd(roomCode, gameResult = {}) {
        ...
        `)
            ...
            const result = await this.bulkUpdatePlayerStatus(roomCode, returnPlayers, 'Game ended - returning players to lobby');
            // Update room status
            await this.lobbyManager.updateRoomStatus(roomCode, 'lobby', 'Game ended');
            // Log game end event
            await this.db.logEvent(roomResult.data.id, null, 'game_ended', {
...
exports.default = StatusSyncManager;
//# sourceMappingURL=statusSyncManager.js.map