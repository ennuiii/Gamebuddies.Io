    ...
    const suppress = (message, args) => {
        try {
            const text = [message, ...(args || [])]
                ...
                .join(' ');
            return (text.includes('HPM WebSocket error') ||
...
class ProxyManager {
    ...
    async setupGameProxies(app, server) {
        ...
        Object.entries(this._gameProxies).forEach(([key, proxy]) => {
            ...
            const proxyConfig = {
                ...
                on: {
                    error: (err, req, res) => {
                        ...
                        const response = res;
                        if (response && !response.headersSent && !req.headers?.upgrade) {
            ...
            const proxyMiddleware = (0, http_proxy_middleware_1.createProxyMiddleware)(proxyConfig);
