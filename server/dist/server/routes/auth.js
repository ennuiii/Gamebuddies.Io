...
const resolveAvatarUrl = (style, seed, options) => {
    if (style === 'custom-mascot' && options?.avatarId) {
        ...
        const baseDir = path_1.default.join(__dirname, '../public/avatars');
        for (const type of types) {
            for (const ext of extensions) {
                const filename = `${id}${ext}`;
                if (fs_1.default.existsSync(path_1.default.join(baseDir, type, filename))) {
...
router.post('/sync-user', async (req, res) => {
    try {
        const { supabase_user_id, email, email_confirmed_at, oauth_provider, oauth_id, avatar_url, display_name } = req.body;
        ...
        const { data: existingUser, error: existingError } = await supabase_1.supabaseAdmin
            .from('users')
            .select('*')
        ...
        const { data: emailUser, error: emailError } = await supabase_1.supabaseAdmin
            .from('users')
            .select('*')
            .eq('email', email)
            .single();
        ...
        if (emailUser) {
            ...
            const { data: migratedUser, error: migrateError } = await supabase_1.supabaseAdmin
                .from('users')
                .update({
        ...
        if (createError) {
            ...
            if (createError.code === '23505') {
                ...
                const { data: retryUser, error: retryError } = await supabase_1.supabaseAdmin
                    .from('users')
                    .insert({
                    id: supabase_user_id,
 ...
 */
router.get('/users/:userId', auth_1.requireAuth, auth_1.requireOwnAccount, async (req, res) => {
    try {
 ...
 */
router.put('/users/avatar', auth_1.requireAuth, async (req, res) => {
    try {
 ...
 */
router.patch('/users/:userId/profile', auth_1.requireAuth, auth_1.requireOwnAccount, async (req, res) => {
    try {
        const { userId } = req.params;
