...
async function fetchStripePrices() {
    ...
    try {
        ...
        const prices = await stripe_1.default.prices.list({
            active: true,
            expand: ['data.product'],
            limit: 100
        });
...
router.post('/create-checkout-session', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (referralCode) {
            const { data: affiliate } = await supabase_1.supabaseAdmin
                .from('affiliates')
                .select('id')
        ...
        const session = await stripe_1.default.checkout.sessions.create(sessionConfig);
...
router.get('/prices', async (req, res) => {
    ...
    try {
        ...
        const [lifetimeDetails, monthlyDetails] = await Promise.all([
            stripe_1.default.prices.retrieve(prices.lifetime, { expand: ['product'] }),
            stripe_1.default.prices.retrieve(prices.monthly, { expand: ['product'] })
        ]);
        const lifetimeProduct = lifetimeDetails.product;
        const monthlyProduct = monthlyDetails.product;
...
router.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        event = stripe_1.default.webhooks.constructEvent(req.body, sig, STRIPE_WEBHOOK_SECRET);
    }
    catch (err) {
 ...
 */
router.post('/customer-portal', auth_1.requireAuth, async (req, res) => {
    try {
 ...
 */
router.post('/cancel-subscription', auth_1.requireAuth, async (req, res) => {
    try {
        const { userId } = req.body;
        if (!userId) {
            res.status(400).json({ error: 'userId is required' });
        ...
        const subscription = await stripe_1.default.subscriptions.update(user.stripe_subscription_id, { cancel_at_period_end: true });
    ...
    if (affiliateId) {
        try {
            ...
            const { data: affiliate } = await supabase_1.supabaseAdmin
                ...
                .single();
            if (affiliate) {
                const typedAffiliate = affiliate;
                const commission = amountPaid * (typedAffiliate.commission_rate || 0.20);
                // 1. Log earnings
                await supabase_1.supabaseAdmin.from('affiliate_earnings').insert({
                    affiliate_id: affiliateId,
                ...
                await supabase_1.supabaseAdmin.from('affiliates').update({
                    total_earnings: (Number(typedAffiliate.total_earnings) || 0) + commission
                }).eq('id', affiliateId);
                // 3. Record referral link (ignore if already referred)
                const { error: referralError } = await supabase_1.supabaseAdmin.from('referrals').insert({
