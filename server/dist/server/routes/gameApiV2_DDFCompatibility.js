...
exports.default = createDDFCompatibilityRouter;
...
const apiKeyMiddleware = typeof validation_1.validateApiKey === 'function'
    ...
    : (req, res, next) => next();
function createDDFCompatibilityRouter(io, db, connectionManager, lobbyManager, statusSyncManager) {
    const router = express_1.default.Router();
    // Primary endpoint used by external games (e.g. DDF) to trigger a return flow.
    router.post('/api/v2/external/return', apiKeyMiddleware, validation_1.rateLimits.apiCalls, async (req, res) => {
        try {
            const apiReq = req;
            const { roomCode, playerId, initiatedBy, reason = 'external_return', returnAll = true, metadata: extraMetadata = {}, } = req.body || {};
            if (!roomCode) {
                res.status(400).json({
                    success: false,
            ...
            try {
                await db.logEvent(typedRoom.id, null, 'external_return_to_lobby', {
    ...
    router.get('/api/v2/rooms/:roomCode/return-status', apiKeyMiddleware, validation_1.rateLimits.polling, async (req, res) => {
        try {
            ...
            if (shouldReturn && playerId) {
                try {
                    sessionToken = await lobbyManager.createPlayerSession(playerId, typedRoom.id, `external_return_${Date.now()}`);
                }
                catch (sessionError) {
                    console.warn('[DDF Compat] Session creation failed:', sessionError);
                }
    ...
    router.get('/api/rooms/:roomCode/validate', apiKeyMiddleware, validation_1.rateLimits.apiCalls, async (req, res) => {
        try {
            const { roomCode } = req.params;
    ...
    router.get('/api/v2/rooms/:roomCode/validate-with-session', apiKeyMiddleware, validation_1.rateLimits.apiCalls, async (req, res) => {
        try {
            const { roomCode } = req.params;
            ...
            if (playerId && validationResult.room?.id) {
                try {
                    sessionToken = await lobbyManager.createPlayerSession(playerId, validationResult.room.id, `external_game_${Date.now()}`);
                }
                catch (sessionError) {
    ...
    router.post('/api/v2/external-heartbeat', apiKeyMiddleware, validation_1.rateLimits.heartbeats, async (req, res) => {
        try {
            const apiReq = req;
            ...
            const result = await statusSyncManager.handleHeartbeat(playerId, roomCode, `external_${Date.now()}`, {
                ...gameData,
...
//# sourceMappingURL=gameApiV2_DDFCompatibility.js.map